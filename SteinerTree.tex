...
\begin{proof}
  ...
  endpoint of $S_j'$ on the line $Y_j'$ (see Proposition~\ref{prop:PaoSte}(ii)). 
  Obviously $S_j'\in \M(\ENCLOSE{H_j'}\cup f_j(A))$.
  ...
  *** By Lemma~\ref{lm:base} we know that $S_j'$ has no branching points
  ...
  Let $d_j(T)$ be the signed distance of $T$ from the line $Y_j$,
  $j=1,2$,
  as defined in Lemma~\ref{lm:tripod} (positive 
  when $T$ is inside $\Delta$).  
  Now we can define a set $S_j''$ which is obtained by adding
  (in case $d_j(T)<0$)
  ...
  In particular the inequalities $\H^1(S_j') \le \H^1(S_j)$ are 
