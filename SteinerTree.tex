...
\begin{figure}
  \centering 
  \includegraphics[height=6cm]{fullTree.png}
  \hspace{1cm}
  \includegraphics[height=6cm]{singleBranch.png}
  \label{fig:tree}
...
Consider the affine functions $f_\pm\colon \CC^2\to \CC^2$ defined by
\[
...
\]
or, by identifying $\CC^2$ with $\RR^2$,
...
If we instead we start from the unit segment $[O,T]$ where $T=(1,0)$, and 
...
\begin{lemma}\label{lemma:triple}
  ...
  Let $A\subset \RR^2$ be a compact set such that 
  $A=A_1\cup A_2\cup A_3$ with $A_k=A\cap B_\rho(V_k)$
...
Let $A_f$ and $\Sigma_f$ be the sets defined in the introduction
(see Figure~\ref{fig:tree} left side)
