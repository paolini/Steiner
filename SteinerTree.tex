...
\begin{lemma}\label{lm:existsA} 
  ...
  Moreover if $S \in \M(\ENCLOSE{x=d}\cup A)$ one has that 
  $\bar S\in \M(\ENCLOSE{x=d}\cup A)$ and, if $\lambda < \frac 1 2$ then 
  $\bar S$ is connected and contains $A$.
\end{lemma}
\begin{proof}
  Follows from Lemma~\ref{lm:exists} with $C:=A$
  minding that $A$ is totally disconnected and if $\lambda < \frac 1 2$ 
  then $\H^1(A)=0$.
\end{proof}
