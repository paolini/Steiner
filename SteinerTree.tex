...
\title{On a Steiner tree connecting a fractal set 
  with positive Hausdorff dimension}
...
segments of $S$.
The infinite tree $S$ is composed by a \emph{trunk} of
...
\begin{figure}
  ...
  \centering 
  \includegraphics[height=4cm]{tree.png}
  \label{fig:tree}
  \caption{The picture is not in scale. **********}
\end{figure}
...
In this paper we further simplify and generalize the method of \cite{CheTep23}
to close the question of both existence and uniqueness. 
...
\[
  A = f_1(A) \cup f_2(A).  
\]
More explicitly $A$ is composed by the limit points of the 
iterations of $f_1$ and $f_2$ starting from $0$.
...
is connected and it is equal to the binary self-similar 
...
\begin{proof}
  ...
  By \cite{PaoSte12} the closure of every connected component 
  ...
  $S\cup C\cup \sigma$ is compact by~\cite{PaoSte12}.
...
Let $\lambda>0$ be fixed, let $f_j$ ($j=1,2$), 
$A$ and $\Sigma$ be as defined in the Introduction.

Let $Y=\ENCLOSE{(x,y)\colon x=0}$ be the $y$-axis,
let $Y_j = f_j(Y)$ for $j=1,2$.
...
%
\begin{proof}
By Lemma~\ref{lm:exists} we know that $\M(\ENCLOSE{x=d}\cup A)$
is nonempty for every $d\in \RR$.
Again by Lemma~\ref{lm:exists} for any $S\in \M(\ENCLOSE{x=d}\cup A)$ 
