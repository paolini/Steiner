...
\begin{figure}
  ...
  junctions. On the left the \emph{universal} fractal tree $\Sigma_u$, 
...
is the unique shortest set which connects $A_u$.
...
and $\H^1(S')<+\infty$ and hence $S'\cup D$ is pathwise connected. 
Consider $\Sigma' = (\Sigma_u\setminus S) \cup A_u \cup \bar S'$.

We claim that $\Sigma'$ is pathwise connected.
Fixed $x_0\in D$ we are going to show that any $x\in \Sigma'$ can be joined to $x_0$ 
by a curve contained in $\Sigma'$.
If $x\in \bar S'$ then $x$ can be joined to $x_0$ with a curve in $\bar S' \subset \Sigma'$ 
because $\bar S'$ is connected (see Lemma~\ref{lm:connected}).
if $x\in \Sigma'\setminus \bar S'$ then $x\in \Sigma_u$ and there exists a curve in $\Sigma_u$ 
joining $x$ to $x_0$: we can restrict such a curve up to the first point $x'\in \bar S'$ and 
reduce to the previous case. The claim is proven.

Since $\Sigma_u\in \M(A_u)$ and $\Sigma'$ is a competitor to $\Sigma_u$ 
we have $\H^1(\Sigma')\le \H^1(\Sigma_u)$
but since $\H^1(\Sigma') \le \H^1(\Sigma_u)-\H^1(S) + \H^1(A_u) + \H^1(\bar S')$ 
we obtain (see Lemma~\ref{lm:connected} again) $\H^1(S)\le \H^1(\bar S') = \H^1(S')$ 
which proves $S\in \M(D)$.
