...
$\rho$ centered at $x$.
We also define $\dist(x,S):=\inf\{\abs{x-y}:y\in S\}$.
...
\begin{lemma}\label{lm:existsA} 
  ...
  Moreover if $\lambda < \frac 1 2$ and $S \in \M(\ENCLOSE{x=d}\cup A)$ 
...
\begin{proof}
  ...
  if $\lambda < \frac 1 2$ 
  then $\H^1(A)=0$ 
  hence $A$ is totally disconnected, 
  and $\H^1(S)\le \H^1(\Sigma) = \frac{1}{1-2\lambda}<+\infty$.
\end{proof}
